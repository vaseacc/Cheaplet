...
export function createMarketBreak(listings, ui) {
    ...
    if (listings.length > 0) {
        listings.forEach(b => {
            ...
            const optimizedImg = typeof window.optimizeImageUrl === 'function' ? window.optimizeImageUrl(img) : img;
            bookHtml += `
...
export function createPostElement(id, d, coll, ui, currentUser, { lang = 'en' } = {}) {
    ...
    let timeStr = d.timestamp ? formatTime(d.timestamp.toDate(), lang) : (ui.justNow || "Just now");
    ...
    const editedLabel = d.edited ? `<span class="edited-badge">${ui.edited_label || ' • Edited'}</span>` : '';
